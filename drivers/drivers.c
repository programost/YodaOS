...
static void vga_scroll_one_line(uint8_t color) {
    for (int i = 1; i <= VGA_LAST_TEXT_ROW; i++)
        for (int j = 0; j < VGA_WIDTH; j++)
            VGA_MEMORY[(i - 1) * VGA_WIDTH + j] = VGA_MEMORY[i * VGA_WIDTH + j];
    for (int j = 0; j < VGA_WIDTH; j++)
        VGA_MEMORY[VGA_LAST_TEXT_ROW * VGA_WIDTH + j] = (color << 8) | ' ';
    vga_y = VGA_LAST_TEXT_ROW;
}

void vga_putchar(char c, uint8_t color) {
    if (vga_y > VGA_LAST_TEXT_ROW)
        vga_y = VGA_LAST_TEXT_ROW;
    if (c == '\b') {
        if (vga_x > 0) {
            vga_x--;
...
void vga_taskbar_refresh(void) {
    ...
    for (j = 0; j < VGA_WIDTH; j++)
        VGA_MEMORY[row * VGA_WIDTH + j] = bar_attr;
    const char *tag = " BB-RAM ";
    for (j = 0; (size_t)j < strlen(tag); j++)
        VGA_MEMORY[row * VGA_WIDTH + j] = (uint16_t)((VGA_COLOR_WHITE << 8) | (uint8_t)tag[j]);
    int barw = VGA_WIDTH - 20;
    if (barw < 8) barw = 8;
    int filled = (pct * barw) / 100;
    if (filled > barw) filled = barw;
    int base = 10;
    for (j = 0; j < filled; j++)
        VGA_MEMORY[row * VGA_WIDTH + base + j] = fill_attr;
    char tail[16];
    int_to_str(pct, tail);
    strcat(tail, "%");
    int tlen = (int)strlen(tail);
    int pos = VGA_WIDTH - 1 - tlen;
    if (pos < base + barw) pos = base + barw;
    for (j = 0; j < tlen; j++)
        VGA_MEMORY[row * VGA_WIDTH + pos + j] = (uint16_t)((VGA_COLOR_LIGHT_GREY << 8) | (uint8_t)tail[j]);
}
...
int disk_find_ext2_partition(uint32_t *out_lba_start, uint32_t *out_sectors) {
    ...
    for (int i = 0; i < 4; i++) {
        if (mbr.partitions[i].type == PARTITION_LINUX_EXT2) {
...
int disk_create_ext2_partition(uint32_t start_lba, uint32_t sectors) {
    ...
    mbr.partitions[0].type = PARTITION_LINUX_EXT2;
    ...
    if (ata_write_sectors(0, 1, &mbr) != 0)
        return -1;
    ata_flush();
    return 0;
}
